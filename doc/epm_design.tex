...
Instructions for setting up an EPM server are in the
{\tt include/maintenance\_parameters.php} file.
...
if you have opened the Project Page
...
initiates and uses sequencing, but other view window pages are single
load pages that do no sequencing.  The help window pages are
...
\FSTACK{admin/}{teams/TID/\\UID.login}}
        & lines
        & \STACK{log of logins\\\pagref{ADMIN/USERS/AID/UID.LOGIN}}
...
\item[\TT{admin/users/AID/UID.login} (login log):]~
\label{ADMIN/USERS/AID/UID.LOGIN} \\
...
EMAIL & Email address (EPM\_EMAIL) used for login \\
IPADDR & IP address (EPM\_IPADDR) for session \\
BROWSER & \STACK{
