...
\begin{enumerate}
\item User IDs (UIDs) are user choosen names.  They are unique to the
      user and used for both external and internal identification.
      Once assigned, they cannot be changed.
\item There are two kinds of UIDs: personal PIDs for individual users,
...
\hline
\TT{admin/+actions+} & lines & \pagref{ADMINISTRATIVE-ACTIONS}
...
\TT{EPM\_EMAIL} & \STACK{login email}
                & login & & all pages \\ 
\hline
\TT{EPM\_TID} & \STACK{login team ID}
                & login & & all pages \\ 
\hline
...
\TT{EPM\_TIME} & \STACK{session\\time}
                 & login & & \STACK{login\\user} \\ 
\hline
\TT{EPM\_ID} & \$ID generation
                 & \STACK{login/problem/view} &
		   \STACK{index\\(creators)} & index  \\ 
\hline
